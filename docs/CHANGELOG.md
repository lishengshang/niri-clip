...
### Fixed
- **>1 MiB 截图无法入库（主捕获路径限流误杀图片，用户实测报告）**：主模式
  `wl-paste --watch` 管道不辨 MIME，截图以原始图片字节直灌 `store`，而入口
  自 1.2 限流起统一按 `max_clip_bytes`（文本 1 MiB）裁断——超文本限即整体
  拒绝，带 `max_image_bytes`（10 MiB）限额的图片路径就此不可达（≤1 MiB 小图
  经非 UTF-8 回退侥幸可达），`max_image_bytes` 对主捕获路径形同虚设，且报错
  误导（文案报的是文本限额数字）。修复：`store_from_stdin` 超限分支先探测
  剪贴板是否提供受支持图片 MIME（png/jpeg/webp，仅探测可用性不读内容），
  是则交图片路径按 `max_image_bytes` 重新裁决（其超限通知自行负责，不叠加
  文本报错）；仅图片不可走（功能关闭/无图片 MIME）时才按文本超限拒绝。
  ≤1 MiB 图片与正常文本行为不变。回归测试 `crates/niri-clip/tests/store_stdin.rs`
  锁定文本超限拒绝（零入库、报文本限额）与正常文本入库；图片正路径依赖
  真实剪贴板内容，须真机 Wayland 会话人工验证
